...
namespace JS_Automation
{
    public class DwgToPdf
    {
        ...
        public static void Run(String dwgFilePath)
        {
            if (string.IsNullOrEmpty(dwgFilePath))
            {
                throw new ArgumentException("message", nameof(dwgFilePath));
            }
            //ExStart:ExportDWGToPDFOrRaster
            // The path to the documents directory.
            //string MyDir = @"C:\Users\D065085\Desktop\BlobFiles\source\S1\";
            //string sourceFilePath = /*MyDir +*/ "Mirror_Sculpture_1_3D.dwg";
            //string outPath = MyDir + "output.pdf";

            using (CadImage cadImage = (CadImage)Image.Load(dwgFilePath))
            {
